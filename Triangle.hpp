...
public:
    ...
    MeshTriangle(objl::Mesh mesh, Material* ip_mat)
    {
        ...
        for (int i = 0; i < mesh.Indices.size(); i += 3) {
            ...
            for (int j = 0; j < 3; j++) {
                auto v_id = mesh.Indices[i + j];
                auto vert = Vector3f(
                    mesh.Vertices[v_id].Position.X,
                    mesh.Vertices[v_id].Position.Y,
                    mesh.Vertices[v_id].Position.Z
                );
